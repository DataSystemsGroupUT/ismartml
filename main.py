...
def progress():
    ...
    if(turn>=iters):
        return render_template("results.html",url_mod=url_mod,column_names=col_names, row_data=res_list,zip=zip,len=len, CLASSIFIERS=ESTIMATORS,CLASSIFIERS_DISP=ESTIMATORS_DISP, estim_dict=estim_dict)
    else:
...
def generate_model():
    ...
    return render_template("download.html",url_mod=url_mod,features=features,targets=np.unique(y),estimator=estim,index=index,column_names=column_names,row_data=metric_res,CL_Name=cl, metric_res=metric_res,zip=zip,partial_fig_names=partial_fig_names)

...
def meta_features2(data, num_cols):
    ...
    if nr_numerical_features != 0:
        ...
        skew_min, skew_q1,         skew_mean, skew_std,         skew_q3, skew_max = summary_stats(skewness_values, 
                                          include_quantiles=True)
        
        kurtosis_min, kurtosis_q1,         kurtosis_mean, kurtosis_std,         kurtosis_q3, kurtosis_max = summary_stats(kurtosis_values,
                                                  include_quantiles=True)
        ...
        try:
            rho_min, rho_mean,             rho_std, rho_max = summary_stats(pairwise_correlations)
        except IndexError:
            pass
...
def meta_features_info_theoretic(X, y):
    ...
    # Class probability    
    metafeatures['prob_min'],     metafeatures['prob_mean'],     metafeatures['prob_std'],     metafeatures['prob_max'] = summary_stats(class_probs)

    # Norm. attribute entropy
    H = norm_entropy(X) 
    metafeatures['norm_entropy_min'],     metafeatures['norm_entropy_mean'],     metafeatures['norm_entropy_std'],     metafeatures['norm_entropy_max'] = summary_stats(H)
    
    # Mutual information
    mutual_information = MI(X, y)
    metafeatures['mi_min'],     metafeatures['mi_mean'],     metafeatures['mi_std'],     metafeatures['mi_max'] = summary_stats(mutual_information)
